        ...
        if (scene) {
            ...
            scene.traverse((object) => {
                if (object instanceof THREE.Mesh) {
                    if(object.material.name.startsWith('roofTop')) roofTopMeshesArray.push(object); // 'roofTop'로 시작하는 Mesh들을 배열에 추가
                    if(object.material.name.startsWith('5F')) f5MeshesArray.push(object); // '5F'로 시작하는 Mesh들을 배열에 추가
                    if(object.material.name.startsWith('4F')) f4MeshesArray.push(object); // '4F'로 시작하는 Mesh들을 배열에 추가
                    if(object.material.name.startsWith('3F')) f3MeshesArray.push(object); // '3F'로 시작하는 Mesh들을 배열에 추가
                    if(object.material.name.startsWith('2F')) f2MeshesArray.push(object); // '2F'로 시작하는 Mesh들을 배열에 추가
                    if(object.material.name.startsWith('1F')) f1MeshesArray.push(object); // '1F'로 시작하는 Mesh들을 배열에 추가
    ...
    useEffect(() => {
        ...
        if(floorName && floorName === 'B1') {
            b1Meshes.forEach((mesh) => {
                if (mesh instanceof THREE.Mesh && mesh.name === 'b1_s1__0') {
                    gsap.to(mesh.position, {
                        z: -100,
                        duration: 0.5, // 애니메이션 시간 (초)
                    });
    ...
    return (
        <>
            ...
            <Canvas className="model-canvas" camera={{position: [7, 8, 10], fov: 20}}>
                ...
                <Model url="/model/noinCenterModeling3.glb" floorName={floorName}/>
                {/* 카메라 위치 제어 */}
